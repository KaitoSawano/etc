...
// or whether the NeroCash app was not started on it.
...
// NeroCash address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.NeroCashAddress)
	if _, err := w.trezorExchange(&trezor.NeroCashGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.NeroCashTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.NeroCashTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
